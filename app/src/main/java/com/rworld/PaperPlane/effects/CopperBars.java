...
public class CopperBars {
    ...
    public void update(float time) {
        ...
        mPosition[0] = mBaseHeight + (int) (mAmplitude * (float) Math.sin(mVelocity));
        mPosition[1] = mBaseHeight + (int) (mAmplitude * (float) Math.sin(mVelocity + FastMath.PI / 4));
        mPosition[2] = mBaseHeight + (int) (mAmplitude * (float) Math.sin(mVelocity + FastMath.PI / 2));
    }
