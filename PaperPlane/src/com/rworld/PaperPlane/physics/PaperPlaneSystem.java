...
public class PaperPlaneSystem extends PhysicSystem {
    ...
    public void update(float time) {
        ...
        emitter.position[0] = mBoundingBox[0] * (float) Math.sin(mStepX);
        emitter.position[1] = mBoundingBox[1] * (float) Math.sin(mStepY) + 0.2f;
        emitter.position[2] = mBoundingBox[2] * (float) Math.sin(mStepX + mStepY);
