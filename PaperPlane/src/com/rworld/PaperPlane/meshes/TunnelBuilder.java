...
public class TunnelBuilder extends MeshBuilder {
	...
	public void updateMeshData(FloatBuffer vertexBuffer, FloatBuffer normalBuffer, FloatBuffer texCoordBuffer, FloatBuffer colorBuffer) {
		final float ct = FloatMath.cos(mTime);
		final float st = FloatMath.sin(mTime);
	...
	protected void build(int stacks, int slices) {
		for(int i = 0; i <= stacks; i++) {
			for(int j = 0; j < slices; j++) {
				...
				e1.vertex[0] = FloatMath.cos(fj) + i * FloatMath.sin(fi) / 4.0f;
				e1.vertex[1] = FloatMath.sin(fj) + i * FloatMath.cos(fi) / 4.0f;
				...
				e6.vertex[0] = FloatMath.cos(fjj) + i * FloatMath.sin(fi) / 4.0f;
				e6.vertex[1] = FloatMath.sin(fjj) + i * FloatMath.cos(fi) / 4.0f;
				...
				e3.vertex[0] = FloatMath.cos(fjj) + (i + 1) * FloatMath.sin(fii) / 4.0f;
				e3.vertex[1] = FloatMath.sin(fjj) + (i + 1) * FloatMath.cos(fii) / 4.0f;
				...
				e4.vertex[0] = FloatMath.cos(fj) + i * FloatMath.sin(fi) / 4.0f;
				e4.vertex[1] = FloatMath.sin(fj) + i * FloatMath.cos(fi) / 4.0f;
				...
				e5.vertex[0] = FloatMath.cos(fjj) + (i + 1) * FloatMath.sin(fii) / 4.0f;
				e5.vertex[1] = FloatMath.sin(fjj) + (i + 1) * FloatMath.cos(fii) / 4.0f;
				...
				e2.vertex[0] = FloatMath.cos(fj) + (i + 1) * FloatMath.sin(fii) / 4.0f;
				e2.vertex[1] = FloatMath.sin(fj) + (i + 1) * FloatMath.cos(fii) / 4.0f;
		...
		for(int i = 0; i < mVertexArray.length; i += 3) {
			mVertexModifier[i + 0] = 0.0025f * (mVertexArray[i + 2] - 4) * FloatMath.cos(mVertexArray[i + 2]);
			mVertexModifier[i + 1] = 0.0025f * (mVertexArray[i + 2] - 4) * FloatMath.sin(mVertexArray[i + 2]);
